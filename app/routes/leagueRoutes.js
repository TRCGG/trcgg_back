...
router.get('/getAllRecord/:riot_name/:guild_id', async (req, res) => {
    ...
    try {
        const data = {
          ...
          month_data: await recordService.getRecentMonthRecord(
            riot_name,
            guild_id
          ),
          ...
          most_pick_data: await championService.getMostPicks(
            riot_name,
            guild_id
          ),
        };
...
router.get('/getRecordWithTeam/:riot_name/:guild_id', async (req, res) => {
    const { riot_name, guild_id } = req.params;
    try {
        const game = await recordService.getSynergisticTeammates(riot_name, guild_id);
        res.json(game);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.get('/master/:champ_name/:guild_id', async (req, res) => {
    ...
    try {
        const master_info = await championService.getMasterInfo(champ_name, guild_id);
...
router.get('/gameStats/:year/:month/:guild_id', async (req, res) => {
    const { year, month, guild_id } = req.params;
    try {
        const game_stats = await recordService.getStatisticOfGame(guild_id, year, month);
        res.json(game_stats);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.get('/lineStats/:position/:guild_id', async (req, res) => {
    ...
    try {
        const line_stats = await recordService.getWinRateByPosition(position, guild_id);
        res.json(line_stats);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.get('/getTopTen/:riot_name/:guild_id', async (req, res) => {
    const { riot_name, guild_id } = req.params;
    try {
        const game = await recordService.getRecentTenGamesByRiotName(riot_name, guild_id);
        res.json(game);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.post('/parse', async (req, res) => {
    ...
    try {
        const result = await replayParsingService.save(file_url, file_name, create_user, guild_id);
        res.json(result);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.post('/mapping', async (req, res) => {
    ...
    try {
        const result = await managementService.postSubAccountName(sub_name, main_name, guild_id);
        res.json(result);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.put('/deleteYn', async (req, res) => {
    ...
    try {
        const result = await managementService.putUserDeleteYN(delete_yn, riot_name, guild_id);
        res.json(result);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.put('/mapping/deleteYn', async (req, res) => {
    const { delete_yn, riot_name, guild_id } = req.body;
    try {
        const result = await managementService.putUserSubAccountDeleteYN(delete_yn, riot_name, guild_id);
        res.json(result);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.put('/riotName', async (req, res) => {
    ...
    try {
        const result = await managementService.putName(new_name, old_name, guild_id);
        res.json(result);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.put('/mapping/riotName', async (req, res) => {
    const { new_name, old_name, guild_id } = req.body;
    try {
        const result = await managementService.putSubAccountName(new_name, old_name, guild_id);
        res.json(result);
    } catch (error) {
        res.status(404).send(error.message);
    }
...
router.delete('/game', async (req, res) => {
    ...
    try {
        const result = await managementService.deleteRecord(
          game_id,
          guild_id
        );
        res.json(result);
...
router.delete('/mapping/subName', async (req, res) => {
    ...
    try {
        const result = await managementService.deleteSubAccountName(riot_name, guild_id);
        res.json(result);
    } catch (error) {
        res.status(404).send(error.message);
    }
